...
import ir.muhammadzadeh.materialkurdishdatetimepicker.multidate.MonthAdapter.CalendarDay;
...
public abstract class MonthView extends View {
    ...
    private final KurdishCalendar mKurdishCalendar;
    protected final KurdishCalendar mDayLabelCalendar;
    ...
    public MonthView(Context context, AttributeSet attr, DatePickerController controller) {
        ...
        mDayLabelCalendar = new KurdishCalendar();
    ...
    public void setMonthParams(HashMap<String, Object> params) {
        ...
        //today.setToNow();
        final KurdishCalendar today = new KurdishCalendar();
        mHasToday = false;
        mToday = -1;

        mKurdishCalendar.setKurdishDate(mYear, mMonth, 1);
        mDayOfWeekStart = mKurdishCalendar.get(Calendar.DAY_OF_WEEK);
    ...
    private boolean sameDay(int day, KurdishCalendar today) {
        return mYear == today.getKurdishYear() &&
                mMonth == today.getKurdishMonth() &&
                day == today.getKurdishDay();
    }
    ...
    protected void drawMonthDayLabels(Canvas canvas) {
        ...
        for (int i = 0; i < mNumDays; i++) {
            ...
            String localWeekDisplayName = mDayLabelCalendar.getKurdishWeekDayName(); // TODO: RTLize
    ...
    private boolean isSelectable(int year, int month, int day) {
        KurdishCalendar[] selectableDays = mController.getSelectableDays();
        for (KurdishCalendar c : selectableDays) {
            if(year < c.getKurdishYear()) break;
            if(year > c.getKurdishYear()) continue;
            if(month < c.getKurdishMonth()) break;
            if(month > c.getKurdishMonth()) continue;
            if(day < c.getKurdishDay()) break;
            if(day > c.getKurdishDay()) continue;
            return true;
        }
        return false;
    }
    ...
    private boolean isBeforeMin(int year, int month, int day) {
        ...
        KurdishCalendar minDate = mController.getMinDate();
        if (minDate == null) {
            return false;
        }

        if (year < minDate.getKurdishYear()) {
            return true;
        } else if (year > minDate.getKurdishYear()) {
            return false;
        }

        if (month < minDate.getKurdishMonth()) {
            return true;
        } else if (month > minDate.getKurdishMonth()) {
            return false;
        }

        if (day < minDate.getKurdishDay()) {
            return true;
        } else {
            return false;
        }
    ...
    private boolean isAfterMax(int year, int month, int day) {
        ...
        KurdishCalendar maxDate = mController.getMaxDate();
        if (maxDate == null) {
            return false;
        }

        if (year > maxDate.getKurdishYear()) {
            return true;
        } else if (year < maxDate.getKurdishYear()) {
            return false;
        }

        if (month > maxDate.getKurdishMonth()) {
            return true;
        } else if (month < maxDate.getKurdishMonth()) {
            return false;
        }

        if (day > maxDate.getKurdishMonth()) {
            return true;
        } else {
            return false;
        }
    ...
    protected boolean isHighlighted(int year, int month, int day) {
        KurdishCalendar[] highlightedDays = mController.getHighlightedDays();
        if(highlightedDays == null) return false;
        for (KurdishCalendar c : highlightedDays) {
            if(year < c.getKurdishYear()) break;
            if(year > c.getKurdishYear()) continue;
            if(month < c.getKurdishMonth()) break;
            if(month > c.getKurdishMonth()) continue;
    ...
    protected class MonthViewTouchHelper extends ExploreByTouchHelper {
        ...
        private final KurdishCalendar mTempCalendar = new KurdishCalendar();
        ...
        protected CharSequence getItemDescription(int day) {
            mTempCalendar.setKurdishDate(mYear, mMonth, day);
            final String date = LanguageUtils.getKurdishNumbers(mTempCalendar.getKurdishLongDate());
