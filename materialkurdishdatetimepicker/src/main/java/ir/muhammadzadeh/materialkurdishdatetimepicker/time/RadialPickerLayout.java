...
public class RadialPickerLayout extends FrameLayout implements OnTouchListener {
    ...
    **/
    ...
    public void initialize(Context context, HapticFeedbackController hapticFeedbackController,
        ...
        for (int i = 0; i < 12; i++) {
            hoursTexts[i] = LanguageUtils.getKurdishNumbers(
                    is24HourMode? String.format("%02d", hours_24[i]) : String.format("%d", hours[i])
            );
            innerHoursTexts[i] = LanguageUtils.getKurdishNumbers(String.format("%d", hours[i]));
            minutesTexts[i] = LanguageUtils.getKurdishNumbers(String.format("%02d", minutes[i]));
        }
    ...
    public boolean dispatchPopulateAccessibilityEvent(AccessibilityEvent event) {
        if (event.getEventType() == AccessibilityEvent.TYPE_WINDOW_STATE_CHANGED) {
            ...
            String timeString = LanguageUtils.getKurdishNumbers(
